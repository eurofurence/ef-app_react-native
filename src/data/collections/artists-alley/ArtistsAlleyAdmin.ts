...
export const artistsAlleyAdminCollection = createCollection(
  queryCollectionOptions({
    ...
    async queryFn({signal}) {
      const response = await api.get<EfTableRegistration[]>('/ArtistsAlley/all', {
        signal,
      })
...
export async function artistsAlleyAdminChangeStatus(id: EfId, status: EfTableRegistrationStatus) {
