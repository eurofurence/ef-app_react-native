 ...
 */
export const useDealerInstances = (t: TFunction, now: Date, items: readonly  DealerDetails[]) => {
    return useMemo(() => {
 ...
 * @param items General results.
 */
export const useDealerAlphabeticalGroups = (t: TFunction, now: Date, items: readonly DealerDetails[]) => {
    return useMemo(() => {
        const day1 = format(setDay(new Date(), 1, { weekStartsOn: 0 }), 'EEEE')
        const day2 = format(setDay(new Date(), 2, { weekStartsOn: 0 }), 'EEEE')
        const day3 = format(setDay(new Date(), 3, { weekStartsOn: 0 }), 'EEEE')
