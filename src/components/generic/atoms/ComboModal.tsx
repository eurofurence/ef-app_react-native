...
export const ComboModal = forwardRef(<T, >({ title, clear, getKey, getLabel, cancelText, confirmText, children }: ComboModalProps<T>, ref: ForwardedRef<ComboModalRef<T>>) => {
    ...
    return <Modal visible={visible} transparent={true} onRequestClose={() => cancel()}>
        <GestureHandlerRootView>
            <View style={[styleDismiss, StyleSheet.absoluteFill]} />
            <View style={[styleBackground, styleBorder, { minHeight: '60%', flex: 1, flexShrink: 1, margin: 20, marginBottom: 90, borderRadius: 20, padding: 20, borderWidth: 2 }]}>
                ...
                <Row>
                    <Button containerStyle={styles.rowLeft} outline={true}
                            onPress={cancel}>
                        {cancelText ?? 'Cancel'}
                    </Button>
                    <Button containerStyle={styles.rowRight} outline={false}
                            onPress={confirm}>
                        {confirmText ?? 'Confirm'}
                    </Button>
                </Row>
