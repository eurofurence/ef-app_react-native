...
export function useFavoritesUpdated() {
  ...
  const {data: events} = useLiveQuery({
    id: 'updated-events',
    query: q => q.from({event: eventsFullCollection})
      .where(({event}) => not(isUndefined(event.Favorite)))
      .innerJoin({view: lastViewTimesCollection}, ({event, view}) => eq(event.Id, view.Id))
      .fn.where(({event, view}) =>
        isAfter(
  ...
  const {data: dealers} = useLiveQuery({
    id: 'updated-dealers',
    query: q => q.from({dealer: dealersFullCollection})
      .where(({dealer}) => not(isUndefined(dealer.Favorite)))
      .innerJoin({view: lastViewTimesCollection}, ({dealer, view}) => eq(dealer.Id, view.Id))
