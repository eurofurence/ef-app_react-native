...
import {eq, inArray, isUndefined, not, or, useLiveQuery} from '@tanstack/react-db'
...
export function DayView({day}: { day: EfDay }) {
  ...
  const grouping = useMemo(() => {
    ...
    return collectBy(sorted, a => isLong(a.Duration) ? 'long_running' : deriveCategorizedTime(a.StartDateTimeUtc))
  }, [events])
  ...
  const listFooterComponent = hidden.length > 0 ? <Label type="lead" variant="middle">
  ...
  return <EfSectionList<string, EfEventFull>
    ...
    renderItem={({item}) => {
      return <EventCard2 containerStyle={styles.item} event={item} onPress={onPressEvent} onLongPress={onLongPressEvent}/>
    }}
