...
export function getTiming(event: EfEvent) {
  ...
  if (result === undefined) {
    const start = toZonedTime(parseDefaultISO(event.StartDateTimeUtc), conTimeZone)
...
export function getProgress(event: EfEvent, now: Date) {
  const durationMs = differenceInMilliseconds(event.EndDateTimeUtc, event.StartDateTimeUtc)
  return differenceInMilliseconds(now, toZonedTime(parseDefaultISO(event.StartDateTimeUtc), conTimeZone)) / durationMs
}
