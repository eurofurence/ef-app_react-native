...
export const CacheProvider = ({ children }: CacheProps) => {
  ...
  useEffect(() => {
    let go = true
    Storage.multiGet(StoreKeys)
      .then((stored) => {
        if (!go) return
        // Hydrate all and dispatch ready state.
        const data: StoreData = { ...initialState }
        for (const [key, value] of stored) if (value !== null) (data as Record<string, any>)[key] = hydrators[key as keyof StoreData](value)
  ...
  const getValue = useCallback(
    <T extends keyof StoreValues>(store: T): StoreValues[T] => {
      return data[store]
    },
    [data]
  )
  ...
  const synchronize = useCallback(async () => {
    ...
    try {
      ...
      function dispatchFromRemote<T extends keyof StoreEntities>(
        store: T,
        change: {
