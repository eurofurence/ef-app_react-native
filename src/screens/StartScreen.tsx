...
const Indi = ({ children }) =>
    <Text style={{ fontSize: 9, color: 'white' }}>{12}</Text>
...
export const StartScreen = () => {
    ...
    const wide = 1000 <= useWindowDimensions().width;
    ...
    return (
        <View style={StyleSheet.absoluteFill}>
            <View style={{ padding: 30 }}>
                ...
                <Button containerStyle={styles.marginAfter} onPress={() => setRead(false)}>Reset read</Button>
                <Button containerStyle={styles.marginAfter} onPress={() => setLoggedIn(false)}>Log-out</Button>
            ...
            <Navigator ref={nav}
                ...
                <Pager ref={pager}
                    left={<View style={{ padding: 30 }}>
                        {loggedIn ? <>
                            <Text style={styles.marginAfter}>You have <Text style={{ fontWeight: 'bold' }}>12</Text> new messages</Text>
                            <Button containerStyle={styles.marginAfter} icon="mail-outline" onPress={on.messages}>Open messages</Button>
                        </> : <>
                            <Text style={styles.marginAfter}>You are currently not logged in</Text>
                            <Button containerStyle={styles.marginAfter} icon="log-in" onPress={on.login}>Log-in now</Button>
                        </>}

                        <Text style={styles.marginAfter}>More locations</Text>
                        {wide ? null : <Button containerStyle={styles.marginAfter} icon="information-circle" onPress={on.info}>Info articles</Button>}
                        {wide ? null : <Button containerStyle={styles.marginAfter} icon="paw" onPress={on.catchEmAll}>Catch-em-all</Button>}
                        {wide ? null : <Button containerStyle={styles.marginAfter} icon="book-outline" onPress={on.services}>Services</Button>}

                        <View style={[styles.marginBefore, styles.row]}>
                            <Button containerStyle={styles.rowLeft} outline icon="cog" onPress={on.settings}>Settings</Button>
                            <Button containerStyle={styles.rowRight} outline icon="card-outline" onPress={on.about}>About</Button>
                        </View>
                    </View>}
                    right={<View style={{ padding: 30 }}>
                        <Text>Enter your username</Text>
                        <TextInput style={[styles.marginAfter, styles.input]} placeholder="Your username" />
                        <Text>Enter your registration number</Text>
                        <TextInput style={[styles.marginAfter, styles.input]} placeholder="Your registration number" />
                        <Text>Enter your password</Text>
                        <TextInput style={[styles.marginAfter, styles.input]} placeholder="Your password" secureTextEntry />
                        <View style={[styles.marginBefore, styles.row]}>
                            <Button containerStyle={styles.rowLeft} outline icon="arrow-back" onPress={on.loginBack}>Back</Button>
                            <Button containerStyle={styles.rowRight} icon="log-in" onPress={on.loginActual}>Log-in</Button>
                        </View>
                    </View>} />
            </Navigator >
        </View >
